    ...
    for column in df.columns:  # 각 컬럼 별 통계값 계산
        ...
        if pd.api.types.is_numeric_dtype(non_null_data):  # 수치형 데이터 case
            column_result.update({
                ...
                '최빈값(mode)': non_null_data.mode()[0] if not non_null_data.mode().empty else None,
            ...
            column_result.update({
                '범주값 길이(고정/가변)': '고정' if non_null_data.apply(
...
# DataFrame -> CSV file
result_df.to_csv("../data_parsing/summary_statistics.csv", index=False, encoding='utf-8-sig')
