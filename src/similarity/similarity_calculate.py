...
# 파일 로드
df = pd.read_csv("../../data_parsing/statistics/summary_statistics_with_classification.csv")

# 수치형, 범주형 컬럼 분리
numerical_cols = ['null 수', '상이값 수', '유일값 수', '값 중복도 평균', '최댓값', '최솟값', '평균(mean)',
...
# 결과 CSV 파일로 저장
pd.DataFrame(numerical_similarity).to_csv("../data_parsing/similarity/similarity_calculate/numerical_similarity.csv", index=False, header=False)
pd.DataFrame(categorical_similarity).to_csv("../data_parsing/similarity/similarity_calculate/categorical_similarity.csv", index=False, header=False)
pd.DataFrame(overall_similarity).to_csv("../data_parsing/similarity/similarity_calculate/overall_similarity.csv", index=False, header=False)
